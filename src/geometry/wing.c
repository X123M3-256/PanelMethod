...
void wing_init_mesh(wing_t* wing,mesh_t* mesh,int chordwise_panels,int spanwise_panels,int wake_length)
{
...
//Wake
int* wake_vertices=calloc(2*spanwise_panels+1,sizeof(int));
segment_t* wake_segments=calloc(2*spanwise_panels,sizeof(segment_t));
int* upper_panels=calloc(2*spanwise_panels,sizeof(int));
int* lower_panels=calloc(2*spanwise_panels,sizeof(int));
	for(int i=0;i<2*spanwise_panels;i++)
	{
	upper_panels[i]=2*i*chordwise_panels;
	lower_panels[i]=2*(i+1)*chordwise_panels-1;
	wake_segments[i].vertices[0]=i;
	wake_segments[i].vertices[1]=i+1;
	}	
	for(int i=0;i<2*spanwise_panels+1;i++)
	{
	wake_vertices[i]=i*section_vertices;
	}
mesh_init(mesh,num_vertices,num_panels,vertices,panels,2*spanwise_panels,2*spanwise_panels+1,wake_vertices,wake_segments,upper_panels,lower_panels,wake_length);
}
